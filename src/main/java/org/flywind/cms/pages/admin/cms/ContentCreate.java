...
public class ContentCreate extends AppBase{
	...
	public void onValidateFromMeForm(){
		...
		try {
			...
			item.setTechnologyType(item.getTechnologyType());
			item.setSmallImgurl(picUrl);
			technologyService.create(item);
			picUrl = null;
		} catch (Exception e) {
			alertManager.error(messages.get("create-error"));
			meForm.recordError(messages.get("create-error"));
		}
	}
